...
router.put('/:exerciseId', async (req, res) => {
	try {
		...
		let targetLibrary =
			entry.exerciseUserLibraryId && mongoose.isValidObjectId(entry.exerciseUserLibraryId)
				? await ExerciseUserLibrary.findOne({
					_id: entry.exerciseUserLibraryId,
					userId: req.userId,
				})
				: null;

		const existingLibraryByName = await ExerciseUserLibrary.findOne({
			userId: req.userId,
			name: { $regex: `^${escaped}$`, $options: 'i' },
		});

		if (existingLibraryByName) {
			targetLibrary = existingLibraryByName;
		}
		...
		if (!targetLibrary) {
			...
			targetLibrary.name = normalizedName;
